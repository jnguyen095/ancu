...
class Post_controller extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['currentGroup'] = $this->session->userdata('usergroup');
		...
		if ($this->input->post('crudaction') == "add_new") {
			...
			if($postToday >= MAX_POST_PER_DAY){
				...
				if(isset($user) && $user->AvailableMoney >= COST_VIP_3_PER_DAY) {
					...
					if(isset($user) && ($user->UserGroupID == USER_GROUP_ADMIN || $user->UserGroupID == USER_GROUP_BROKER || $user->UserGroupID == USER_GROUP_STAFF)){
						$this->load->view('post/new', $data);
					}
	...
	private function processSaveNewPost($data, $type){
		try{
			...
			$data['postCost'] = 0;
			$data['displayMap'] = 0 ;// $this->input->post("displayMap");
			...
			$this->form_validation->set_rules("txt_city", "Thành phố", "required|numeric|greater_than[0]");
			$this->form_validation->set_rules("txt_district", "Quận", "required|numeric|greater_than[0]");
			$this->form_validation->set_rules("txt_ward", "Phường", "numeric|greater_than[0]");
			...
			if($data['user']->UserGroupID == USER_GROUP_CUSTOMER){
