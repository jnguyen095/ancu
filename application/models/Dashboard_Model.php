...
class Dashboard_Model extends CI_Model
{
	...
	public function countUser(){
		$query = "select count(*) as Total from us3r where UserGroupID = " . USER_GROUP_CUSTOMER;
		$total = $this->db->query($query);
		$row = $total->row();
		return $row->Total;
	}
	...
	public function countCrawlerBySource($isToday){
		$today = date('Y-m-d');
		$query = "select pd.Source, count(*) as Total from productdetail pd INNER JOIN product p where p.ProductID = pd.ProductID";
		if(isset($isToday) && $isToday){
			$query .= " and date(p.PostDate) = '{$today}'";
		}
		$query .= " GROUP BY pd.Source";
		$result = $this->db->query($query);
		return $result->result();
	}
